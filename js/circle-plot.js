...
Assembly.prototype.drawPlot = function(parent_div, longest, circle_span) {
  ...
  if (this.busco) {
    ...
    key.append('rect').attr('y', w * 3).attr('height', w).attr('width', w).attr('class', 'asm-busco_M asm-toggle');
    key.append('text').attr('x', w + 3).attr('y', w * 6.5 - 1).text('Miss. (' + this.busco.M.toFixed(1) + '%)').attr('class', 'asm-key');
  }
  else if (this.cegma_complete) {
    var lccg = lg.append('g')
